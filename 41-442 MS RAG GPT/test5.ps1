...
$location      = if ($env:LAB_LOCATION) { $env:LAB_LOCATION } else { "eastus2" }
...
Write-Log "Setting subscription to $subscriptionId location $location."
azd env set AZURE_SUBSCRIPTION_ID $subscriptionId | Out-String | Write-Log
azd env set AZURE_LOCATION $location | Out-String | Write-Log
